...
if args.generateConfig:
    ...
    logfile = open("log_tempAlevels.log", "w")
    logfile.write(str(now()) + ' start\n')

    errfile = open("err_tempAlevels.err", "w")

    with open("config_tempAlevels.json", "w") as outfile:
        json.dump(obj, outfile, indent=4)
        logfile.write(str(now()) + ' config file generated and end\n')
        sys.exit("config file generated")

if args.configFile == None:
    args.configFile = "config_tempAlevels.json"
